...
V-Dem reports several regime identifiers that can be used to mark institutional continuity and rupture. We take a **state-centered** approach. Rather than adopting `v2reginfo`, which splits political history into leader- or phase-specific episodes (e.g., “USSR under Stalin”), we track the persistence of the overarching political unit via `histname`. Concretely, within each country we standardize `histname` and flag a **regime change this year** when it differs from the previous year. This choice treats leader rotations and intra-regime episodes as continuity while capturing foundational transformations such as refounding, merger, or dissolution. The next code chunk implements this step and, crucially for prediction, shifts the indicator one year forward to construct a **t to t+1** target (i.e., whether a change occurs next year, given information available at year t).

Beyond whether a regime changes, we also care about **how** it ends. V-Dem’s `v2regendtype` records the process judged most important for terminating a regime in a given year. We align this process label to the design by defining `regend_t1` as the **next year’s** value of `v2regendtype`. From `regend_t1` we build the panel-wide outcome:
- $Y*_{t+1} = 1$ if `regend_t1 ∈ {0, 1, 2}` (military coup, non-military coup, self-coup);
- $Y*_{t+1} = 0$ for **all other non-missing** values, including “regime still exists” (`13`).

This construction yields an outcome for **every** country–year and, by not restricting attention to cases that end, **avoids conditioning on termination**—a common collider that can bias comparisons of “coup vs. non-coup among endings.”

The treatment is defined symmetrically at year t. Using `e_pt_coup_attempts`, we code `A_t = 1` when at least one coup attempt is recorded (count > 0) and `A_t = 0` otherwise. Because attempt coverage is strongest after the mid-20th century, the effective sample tilts toward that period; the data-preparation code filters missing values accordingly so that `A_t` and `Y*_{t+1}` are temporally aligned and jointly observed.

Before turning to causal models, we report **descriptive priors** based on `v2regendtype`. First, we summarize the overall distribution of termination types to show that coup endings are comparatively rare and that “still exists” is common. Second, we examine the mix of types among observations that precede a `histname` change (aligned to `t+1`). These summaries calibrate prior beliefs about base rates and mechanisms, yet they are **not** used as conditioning variables in the causal stage; they serve only as context and a check on measurement.

Finally, we include two simple diagnostics that connect measurement to the subsequent design. We display the base rate of within-country `histname` changes and spot-check a handful of flagged rows to verify that changes correspond to visible `histname` shifts. These checks, together with the year-forward alignment of `regend_t1`, establish the target we seek to explain—**coup termination next year**—and prepare the ground for the identification strategy that follows (country and year effects, plus a time-varying confounder).
...
Before modelling, we summarize how regimes end in V-Dem—both overall and among years that precede a `histname` change. These tables are descriptive only; they provide useful priors (e.g., coups are rare relative to other endings).We define regime change as a change in the `histname` field within a country across years.
