...
df_yx <- vdem_change %>%
  mutate(
    ...
    # ① 企图次数（当年 t）
    coup_attempts_t = e_pt_coup_attempts,
    # ② 二值化：是否有企图（成功与否都算）
    x_attempt = ifelse(!is.na(coup_attempts_t) & coup_attempts_t > 0, 1L, 0L)
  ) %>%
  filter(!is.na(y)) %>%                  # 跟 y 对齐
  select(y, x_attempt)
...
现在的 baseline **只有一个全局概率** pcoupp_{\text{coup}}pcoup​，它回答的是：
