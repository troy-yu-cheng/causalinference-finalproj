...
alpha, beta = 20*0.08, 20*(1-0.08)  # 举例：先验中心8%，等效样本量20 —— 先暂时占位
with pm.Model() as beta_bern:
    p_coup = pm.Beta("p_coup", alpha=alpha, beta=beta)        # 顶部：先验参数
    coup_t1 = pm.Bernoulli("coup_t1", p=p_coup, observed=y)   # 中间+底部：生成+观测
# 画图（会得到和作业一致的三节点）
pm.model_to_graphviz(beta_bern)
